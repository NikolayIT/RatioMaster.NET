...
namespace BitTorrent
{
	internal class Torrent 
	{
		...
		{
            get { return (((ValueDictionary)data["info"]).Contains("length")); }
        }
        ...
        {
            get { return (ValueDictionary)data["info"]; }
        }
		...
		private void ParseSingleFile()
		{
			...
			TorrentFile f = new TorrentFile(((ValueNumber) info["length"]).Integer, 
			                                ((ValueString) info["name"]).String);
			torrentFiles.Add(f);
		}
		...
		private void ParseMultipleFiles()
		{
			...
			foreach(ValueDictionary o in files)
			{
                ...
                TorrentFile f = new TorrentFile(((ValueNumber) o["length"]).Integer,
				                  path);
				torrentFiles.Add(f);
			}
		}
	...
	{
        internal IncompleteTorrentData(string message) : base(message) { }
	}
